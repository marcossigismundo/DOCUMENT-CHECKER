...
function tcd_enqueue_admin_assets( string $hook ): void {
	...
	wp_enqueue_script(
		...
		array( 'jquery' ),
		filemtime( TCD_PLUGIN_DIR . 'assets/js/admin.js' ),
		true
	);
	...
	wp_enqueue_style(
		...
		array(),
		filemtime( TCD_PLUGIN_DIR . 'assets/css/admin-style.css' )
	);
}
...
function tcd_enqueue_frontend_assets(): void {
	...
	if ( has_shortcode( $post->post_content, 'tainacan_doc_status' ) ) {
		wp_enqueue_style(
			'tcd-frontend',
			TCD_PLUGIN_URL . 'assets/css/frontend-style.css',
			array(),
			filemtime( TCD_PLUGIN_DIR . 'assets/css/frontend-style.css' )
		);
	}
}
